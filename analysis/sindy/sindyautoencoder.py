...
def autoencoder_pass(params, x, dx):
    ...
    sindy_predict = jnp.matmul(Theta, params['sindy_coefficients'])
...
def loss(sindy_autoencoder, input, dx_input):

    activations = zip(*[autoencoder_pass(sindy_autoencoder, input[i, :], dx_input[i, :]) for i in range(input.shape[0])])
    x, dx, z, dz, x_decode, dx_decode, Theta, sindy_predict = map(jnp.stack, activations)

    reconstruction_loss = jnp.mean((x - x_decode)**2, axis=-1)
    sindy_z = jnp.mean((dz-sindy_predict)**2, axis=-1)
    sindy_x = jnp.mean((dx - dx_decode)**2, axis=-1)

    total_loss = reconstruction_loss + sindy_z + sindy_x
    return total_loss

@jit
def update(params, inpuz, dx_input, opt_state):

    value, grads = value_and_grad(loss)(params, input, dx_input)

if __name__ == "__main__":
    layer_sizes = [10, 5, 5, 3]
    key = random.PRNGKey(1)

    sindy_autoencoder = build_sindy_autoencoder(layer_sizes, key)

    batch_shape = (20, 10)
    input = random.normal(key, batch_shape)
    dx_input = random.normal(key, batch_shape)
