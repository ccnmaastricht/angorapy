...
def reset_states_masked(recurrent_layers: List[tf.keras.Model], mask: List):
    ...
    for layer in recurrent_layers:
        ...
        for current_state in current_states:
            expanded_mask = numpy.tile(numpy.flip(numpy.rot90(numpy.expand_dims(mask, axis=0))), (1, current_state.shape[-1]))
...
def make_input_layers(env, bs=None, sequence_length=None) -> tuple[dict[str, tf.keras.Input], dict[str, tf.keras.layers.Masking]]:
    ...
    for modality in state_dimensionality.keys():
        ...
        if sequence_length is not None:
            the_masked_input = tf.keras.layers.Masking(batch_input_shape=shape, name=f"masked_{modality}")(the_input)
