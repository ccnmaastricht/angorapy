...
class Avemover:
    ...
    def gen_data(self, nPulses: int = 100):
        ...
        pulse_beginnings = np.random.randint(0, seconds, lenP)
        ...
        for i in range(pulse_beginnings.shape[0]):
            ...
            if time_difference <= 30 & time_difference >= 5:
                input[int(pulse_beginnings[i]/delta_t):(int(pulse_beginnings[i]/delta_t)+lenP), 0, 1] = np.ones(lenP)
                target[int(pulse_beginnings[i]/delta_t-2*targetpulse):int(pulse_beginnings[i]/delta_t-targetpulse)] = np.repeat(np.mean((pulses[0, (i-1)], pulses[0, (i-2)])), targetpulse)
